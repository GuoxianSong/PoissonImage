...
// 	action_new_->setStatusTip(tr("Create a new file"));
	...
	QStringList shapes;
	shapes<<tr("Rectangle")<<tr("Ellipse")<<tr("Polygon");
	...
	QStringList pasteMethods;
	pasteMethods<<tr("Copy")<<tr("Poisson")<<tr("MixPoisson");
	combobox_paste_ = new QComboBox(this);
	combobox_paste_->addItems(pasteMethods);
	connect(combobox_paste_, SIGNAL(activated(int)), this, SLOT(Paste()));

	action_paste_ = new QAction(tr("Paste"), this);
	connect(action_paste_, SIGNAL(triggered()), this, SLOT(Paste()));

	action_test_inside_points_ = new QAction(tr("test"),this);
	connect(action_test_inside_points_,SIGNAL(triggered()),this,SLOT(TestInsidePoints()));
}
...
//	toolbar_file_->addAction(action_new_);
	...
	toolbar_file_->addSeparator();
	toolbar_file_->addWidget(combobox_shape_);
	toolbar_file_->addAction(action_choose_);
	toolbar_file_->addWidget(combobox_paste_);
	toolbar_file_->addAction(action_paste_);
	//toolbar_file_->addAction(action_test_inside_points_);		//²âÊÔÉ¨ÃèÏßÄÚµã
}
...
void MainWindow::Choose()
{
	...
	window->imagewidget_->set_region_shape(combobox_shape_->currentIndex());
...
void MainWindow::Paste()
{
	...
	child->imagewidget_->set_paste_method(combobox_paste_->currentIndex());
	child->imagewidget_->set_source_window(child_source_);
	if (combobox_paste_->currentIndex())
		child->imagewidget_->matrix_predecomp();
}
